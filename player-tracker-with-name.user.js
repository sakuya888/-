// ==UserScript==
// @id             iitc-plugin-player-tracker-with-name@test
// @name           IITC Plugin: Player tracker with name
// @category       Layer
// @version        0.00.1.20190311.163611
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @updateURL      none
// @downloadURL    none
// @description    [local-2019-03-11-163611] Draw trails for the path a user took onto the map based on status messages in COMMs. Uses up to three hours of data. Does not request chat data on its own, even if that would be useful.
...
plugin_info.dateTimeVersion = '20190311.163611';
plugin_info.pluginId = 'player-tracker-with-name';
...
window.plugin.playerTracker.drawData = function() {
  ...
  $.each(plugin.playerTracker.stored, function(plrname, playerData) {
    ...
    var agentName = plrname + "(" + ago(last.time, now) + ")";
    var divIcon = L.divIcon({
      className: "labelClass",
      html: "<span style='font-size: 14pt; white-space:nowrap; text-shadow: 0 0 0.1em black, 0 0 0.1em black, 0 0 0.1em black,0 0 0.1em black,0 0 0.1em; color:" + (playerData.team === 'RESISTANCE' ? 'DeepSkyBlue' : 'LawnGreen') + "; position:absolute; top:-60px; left:-" + (agentName.length / 2) * 9 + "px; font-family:Consolas,Monaco,Lucida Console,Liberation Mono,DejaVu Sans Mono,Bitstream Vera Sans Mono,Courier New, monospace;'>" + agentName + "</span>"
    });
